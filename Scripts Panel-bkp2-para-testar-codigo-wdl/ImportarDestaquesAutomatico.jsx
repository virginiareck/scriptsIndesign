...
//2017-10-02  - myNumero = 2; para nao atrapalhar o script do sumario
...
var destaqueAutomatico = app.scriptArgs.get('DestaqueAutomatico');
...
function main(){
    ...
    if(!destaqueAutomatico){
        pegarPagina();
        myFile = File.openDialog ("Selecione o Arquivo PDF Destaque");
    }else{
        myNumero = 2;
        myURLField = "https://ciga.sc.gov.br/diario-oficial-dos-municipios-qr-code/";
        myPgQR = false;
        myPgCabecalho = false;
        myPgInteira = true;
        myFile = new File (destaqueAutomatico) ;
    }
    
	myNumero--;
    myPage = myDocument.pages.item(myNumero);
	//myFile = destaqueAutomatico? new File (destaqueAutomatico)  : File.openDialog ("Selecione o Arquivo PDF Destaque");
                ...
                with(dialogColumns.add()){
                    ...
                    var myRBG1 = radiobuttonGroups.add();
						with(myRBG1){
							...
							var myVD1Cabecalho = radiobuttonControls.add({staticLabel:"Cabeçalho", minWidth:280});
							var myVD1Inteira = radiobuttonControls.add({staticLabel:"Inteira", checkedState:false,alignment:'left'});
...
function paginaCabecalho(myDocument, myFrame){
    var myFrameBounds = [45.3,5.3,289.65,204.65];
    myFrame.geometricBounds = myFrameBounds;
    myFrame.fit(FitOptions.proportionally);
    myFrame.fit(FitOptions.frameToContent);
    myFrame.parent.appliedMaster = myDocument.masterSpreads.item("B-Capa");
