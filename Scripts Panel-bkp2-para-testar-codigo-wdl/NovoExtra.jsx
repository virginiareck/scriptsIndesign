...
var img_extra =  '/Logos/extra-manus.png';
var sigla = 'DOM/SC';
...
function identificaVersao(){
   ...
   if(printDebug) $.writeln("aqui")
  if(printDebug)  $.writeln(tmp_obj.attributes());
   if (prop.child("sigla")){
...
function configuraVersao(sigla){   
   if (sigla == "DOM/ITAJAI"){
       ...
       img_logo = '/Logos/domitajai/logo_domitajai.png';
...
function estilos() {
   ...
   if (sigla == "DOE-TCE/SC"){
/*        with (styles.item("entidade - Estadual")) {
               keepWithNext = 5;
               keepAllLinesTogether = true;
               spanColumnType = SpanColumnTypeOptions.SPAN_COLUMNS;
               spanSplitColumnCount = SpanColumnCountOptions.ALL;
...
function importarXML() {
    ...
    for(var g = 0; g < grupos.length; g++) {
        ...
        if (nomeGrupo == "Consórcios Intermunicipais" || nomeGrupo == "Consórcios" || nomeGrupo == "ADMINISTRAÇÃO PÚBLICA ESTADUAL") {
...
function inserirImagem(alvo, posicao, arquivo) {
    $.writeln("inserirImagem.alvo: " + alvo);
    $.writeln("inserirImagem.posicao: " + posicao);
    $.writeln("inserirImagem.arquivo: " + arquivo);
...
function addFPaisagem(){
   ...
   if (sigla == "DOM/ES" || sigla == ' ' || sigla == null){
	// Add Edição
	...
	$.writeln(sigla)
 	var ed = inserirFrameTexto (pagina, [6.5, 65, 12, 140], "DOM/SC - Edição Extra N° "+edicao, 8, Justification.centerAlign);
   	ed.rotationAngle = 90;
   	ed.move([6.5, 170]);
   }
   //Add Data   
...
function addBCapa(){
   ...
   if (sigla == "DOE-TCE/SC" || sigla == "DOM/ITAJAI"){
  ...
  //  script_file.execute(edicao);
   ...
   //paginaExtra = myDoc.pages[0];
   paginaExtra = myDoc.pages.item(0);
   //inserirImagem (paginaExtra, [19,11,188.8,201.5], rootFolder + img_extra);
   
   //chamar ImportarTextoEdicaoExtra.jsx
   //inseir texto edicao extra
  /* rootFolder = "D:/DOMSC/Edição 3093";
   var scrTextoExtra = File(rootFolder + "/ImportarTextoEdicaoExtra.jsx");
    if(printDebug)  $.writeln("Novo Extra : var scrTextoExtra: " + scrTextoExtra);
    var scriptTextoExtra = '#include ' + scrTextoExtra.fullName;
    if(printDebug)  $.writeln("Novo Extra : var scriptTextoExtra: " + scriptTextoExtra);
    //eval(scriptTextoExtra);
  // CallClient("ScriptingSupport", scrTextoExtra);
  app.doScript (scriptTextoExtra, ScriptLanguage.JAVASCRIPT, [ 0,  0 ] );  */
   ...
   // Edição
   inserirFrameTexto (pagina, [38+offset_y, 11, 45+offset_y, 198], "Edição Extra N° "+edicao, 12, Justification.leftAlign);
   ...
   inserirFrameTexto (pagina, [6.5, 8, 12, 50], dia+"/"+mes+"/"+ano+" ("+semana[diaSemana-1]+")", 8, Justification.leftAlign);
	...
	if(sigla == 'DOM/ES'){
		inserirFrameTexto (pagina, [6.5, 55, 12, 155], sigla+" - Edição N° "+edicao, 8, Justification.centerAlign);
   		...
   		inserirFrameTexto (pagina, [6.5, 55, 12, 155], "DOM/SC - Edição Extra N° "+edicao, 8, Justification.centerAlign);
	}
   // Site
